...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      if (Array.isArray(body)) {
        ...
        return NextResponse.json(
          { version: 'v1', error: 'Expected JSON array or { items: [...] } format' },
          { status: 400 }
        );
      }
    } else {
    ...
    if (!items.length) {
      return NextResponse.json({ version: 'v1', error: 'No coordinate items provided.' }, { status: 400 });
    }

    if (items.length > 2000) {
      return NextResponse.json(
        { version: 'v1', error: 'Batch limit exceeded. Maximum 2,000 items per request.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { version: 'v1', error: 'Failed to process batch request', details: error?.message },
      { status: 500 }
    );
  }
}
